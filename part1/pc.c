...
uthread_mutex_t mx;
uthread_cond_t producerCanGo, consumerCanGo;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    ...
    while (items >= MAX_ITEMS){
      uthread_cond_wait(producerCanGo);
    }
    items += 1;
    uthread_cond_signal(consumerCanGo);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_mutex_lock(mx);
    while (items == 0) {
      uthread_cond_wait(consumerCanGo);
    }
    items -= 1;
    uthread_cond_signal(producerCanGo);
    assert(items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;  
    uthread_mutex_unlock(mx);
  }
...
int main (int argc, char** argv) {
  ...
  // init mutex and cond threads
  mx = uthread_mutex_create();
  producerCanGo = uthread_cond_create(mx);
  consumerCanGo = uthread_cond_create(mx);
